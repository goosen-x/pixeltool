...
import { onlineToolsCountLabel } from '@/lib/utils/pluralize'
...
// Четыре ссылки на категории, а не весь каталог: пятнадцать пунктов занимали
// весь правый край экрана и читались как оглавление, а не как быстрый вход.
// Остальное закрывает кнопка «Все инструменты» под заголовком.
//
// Выбор не по вкусу и не по алфавиту. Вордстат и реальные просмотры сильно
// расходятся, поэтому взяты категории, сильные по обоим сразу: «Изображения и
// PDF» первые и по показам (27,9%), и третьи по спросу (14,4%); «Рандомайзер»
// вторые по показам (20,2%) и ровно то, что обещает заголовок героя; «Текст»
// третьи по показам (15,2%) при заметном спросе; «Математика» первая по спросу
// (22,9%) и держится здесь как ставка на рост, просмотров там пока мало только
// потому, что раздел собран недавно.
//
// Главное, чем пришлось пожертвовать: «Эзотерика» это 19,5% спроса, но
// «гадания» на первом экране меняют позиционирование сайта, и это решение не
// арифметическое. «Разработка» даёт 14,3% просмотров, но всего 1,9% спроса.
//
...
const navLinks = HERO_CATEGORIES.map((key, i) => ({
				...
				<div className='absolute left-0 top-0 z-20 flex min-h-20 items-center rounded-br-3xl bg-background py-4 pl-6 pr-6 sm:pl-8'>
					...
					<div className='relative z-10 leading-tight'>
						<span className='text-sm font-medium text-muted-foreground'>
							{onlineToolsCountLabel(publicWidgets.length)}
						</span>
					</div>
				</div>
