...
export function WidgetKeyboardShortcuts({
  ...
  const ShortcutKey = ({ children, variant = 'default' }: { children: React.ReactNode, variant?: 'default' | 'outline' }) => (
    <kbd className={cn(
          ...
          <div className="absolute bottom-full right-0 mb-2 animate-in fade-in slide-in-from-bottom-2">
            <Card className="p-3 pr-8 shadow-lg">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>Press <ShortcutKey>?</ShortcutKey> for shortcuts</span>
              </div>
            </Card>
          </div>
        )}
...
export function ShortcutHint({ shortcut, className }: { shortcut: KeyboardShortcut, className?: string }) {
  const modifiers = formatModifiers(shortcut)
  const key = formatKey(shortcut.key)
  const isMac = typeof window !== 'undefined' && /Mac|iPod|iPhone|iPad/.test(window.navigator.platform)
