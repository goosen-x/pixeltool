...
export function AnalyticsDashboard({ widgets, locale }: AnalyticsDashboardProps) {
  ...
  const [allWidgetStats, setAllWidgetStats] = useState<Map<string, WidgetStats>>(new Map())
  ...
  const fetchWidgetStats = async (widgetId: string) => {
    setLoading(true)
    try {
      const response = await fetch(`/api/analytics/stats/${widgetId}?timeframe=${timeframe}`)
      if (response.ok) {
  ...
  const fetchAllStats = async () => {
    const statsPromises = widgets.map(async (widget) => {
      try {
        const response = await fetch(`/api/analytics/stats/${widget.id}?timeframe=${timeframe}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const totalStats = {
    views: Array.from(allWidgetStats.values()).reduce((sum, stats) => sum + stats.totalViews, 0),
    sessions: Array.from(allWidgetStats.values()).reduce((sum, stats) => sum + stats.totalSessions, 0),
    onlineUsers: Array.from(allWidgetStats.values()).reduce((sum, stats) => sum + stats.onlineUsers, 0),
    avgDuration: Array.from(allWidgetStats.values()).reduce((sum, stats) => sum + stats.averageSessionSeconds, 0) / (allWidgetStats.size || 1)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{dashT('totalViews')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{dashT('totalSessions')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{dashT('avgDuration')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{dashT('onlineNow')}</CardTitle>
            ...
            {topWidgets.map(({ widget, stats }, index) => (
              <div key={widget.id} className="flex items-center">
                <div className="flex items-center gap-3 flex-1">
                  <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${widget.gradient} flex items-center justify-center text-white`}>
      ...
      <Tabs defaultValue="widget" className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Select value={timeframe} onValueChange={(value: '1d' | '7d' | '30d') => setTimeframe(value)}>
            ...
            <CardContent>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {widgets.map((widget) => (
                  <Button
                    key={widget.id}
                    variant={selectedWidget?.id === widget.id ? 'default' : 'outline'}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">{dashT('viewsToday')}</span>
                    <span className="font-medium">{widgetStats.viewsToday.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">{dashT('totalViews')}</span>
                    <span className="font-medium">{widgetStats.totalViews.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">{dashT('uniqueSessions')}</span>
                    <span className="font-medium">{widgetStats.totalSessions.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">{dashT('avgSessionDuration')}</span>
                    <span className="font-medium">{widgetStats.averageSessionDuration}</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <BarChart data={topWidgets}>
                  ...
                  <XAxis 
                    ...
                    tickFormatter={(id) => {
                      ...
                      return widget ? t(`${widget.translationKey}.title`).slice(0, 15) + '...' : id
                    }}
                  />
                  <YAxis />
                  <Tooltip 
