...
export const getWidgetById = unstable_cache(
  async (id: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget | null> => {
    const { data, error } = await supabase
      .from('widgets')
      .select(`
        *,
...
export const getWidgetBySlug = unstable_cache(
  async (slug: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget | null> => {
    const { data, error } = await supabase
      .from('widgets')
      .select(`
        *,
...
export const getWidgetsByCategory = unstable_cache(
  async (category: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget[]> => {
    const { data, error } = await supabase
      .from('widgets')
      .select(`
        *,
...
export const getWidgetFAQs = unstable_cache(
  async (widgetId: string, locale: 'en' | 'ru' = 'en'): Promise<{ question: string; answer: string }[]> => {
    const { data, error } = await supabase
      .from('widget_faqs')
...
export const getPopularWidgets = unstable_cache(
  async (locale: 'en' | 'ru' = 'en', limit: number = 6): Promise<LocalizedWidget[]> => {
    const { data, error } = await supabase
      .from('widgets')
      .select(`
        *,
