...
export function generateGradientCSS(settings: GradientSettings): string {
	...
	switch (type) {
		...
		case 'radial': {
			const { radialShape, radialSize, radialPositionX, radialPositionY } =
				settings
...
export function getGradientCategories(): string[] {
	const categories = new Set(
		PRESET_GRADIENTS.map(gradient => gradient.category)
	)
...
export function searchGradients(query: string): PresetGradient[] {
	...
	return PRESET_GRADIENTS.filter(
		gradient =>
