...
export function getStaticWidgetById(id: string, locale: 'en' | 'ru'): LocalizedWidget | null {
  return STATIC_WIDGETS_DATA[locale].find(w => w.id === id) || null
}

export function getStaticWidgetBySlug(slug: string, locale: 'en' | 'ru'): LocalizedWidget | null {
  return STATIC_WIDGETS_DATA[locale].find(w => w.slug === slug) || null
}

export function getStaticWidgetsByCategory(category: string, locale: 'en' | 'ru'): LocalizedWidget[] {
  return STATIC_WIDGETS_DATA[locale].filter(w => w.category === category)
}

export function getStaticPopularWidgets(locale: 'en' | 'ru', limit: number = 6): LocalizedWidget[] {
  return STATIC_WIDGETS_DATA[locale]
    .filter(w => w.is_popular)
    .slice(0, limit)
}

export function getStaticNewWidgets(locale: 'en' | 'ru', limit: number = 6): LocalizedWidget[] {
  return STATIC_WIDGETS_DATA[locale]
    .filter(w => w.is_new)
    .slice(0, limit)
}

export function getStaticWidgetsByTag(tag: string, locale: 'en' | 'ru'): LocalizedWidget[] {
  return STATIC_WIDGETS_DATA[locale].filter(w => w.tags.includes(tag))
}

export function getStaticWidgetFAQs(widgetId: string, locale: 'en' | 'ru'): { question: string; answer: string }[] {
