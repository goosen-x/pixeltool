...
export const SEO_REDIRECTS = [
	...
	// Склейка статьи с инструментом: /blog/kalkulyator-bitkoina-v-rubli состояла
	// из 317 слов, и три её раздела («Откуда берётся курс», «Считает в обе
	// стороны», «Почему только три валюты») дословно повторяли три FAQ тула.
	// Заголовок вдобавок начинался с головной фразы инструмента — обе страницы
	// делили один запрос. Единственная уникальная деталь, про расхождение
	// агрегированного курса с ценой конкретной биржи, перенесена в FAQ.
	{
		source: '/blog/kalkulyator-bitkoina-v-rubli',
		destination: '/tools/bitcoin-calculator',
		statusCode: 301
	},
	...
	// Демо-пост «Интерактивные примеры кода» удалён, но страница успела
	// проиндексироваться и без редиректа отдавала 200 с заглушкой
	// «Статья не найдена» вместо настоящего 404.
	{
		source: '/blog/live-code-demo',
		destination: '/blog',
		statusCode: 301
	},

	// php-syntax-checker и mysql-syntax-checker удалены: органического спроса
	// под эти формулировки почти нет (1 и 0 показов/мес по Вордстату),
	// но оба URL были в sitemap и в очереди на переобход — ведём на категорию,
	// а не оставляем 404.
	{
		source: '/tools/php-syntax-checker',
		destination: '/tools/utilities',
		statusCode: 301
	},
	{
		source: '/tools/mysql-syntax-checker',
		destination: '/tools/utilities',
		statusCode: 301
	},

	// seo-markdown-generator удалён (нулевой спрос, см. коммит 885113b), но
	// редирект тогда не завели — Яндекс.Вебмастер до сих пор его знает и
	// получал честный 404 (найдено в отслеживании важных страниц 16.08.2026).
	{
		source: '/tools/seo-markdown-generator',
		destination: '/tools/utilities',
		statusCode: 301
	},

	// flip-text удалён 17.08.2026 как неиспользуемый демо-тул, но успел
	// проиндексироваться и получить внутренние ссылки со страниц
	// flexbox-generator и regex-tester (найдено в GSC «Проверка URL» —
	// 404, ссылающиеся страницы ещё не пересканированы) — ведём на
	// ближайший по смыслу тул той же категории «Текст» вместо 404.
	{
		source: '/tools/flip-text',
		destination: '/tools/fancy-text-generator',
		statusCode: 301
	}
]
