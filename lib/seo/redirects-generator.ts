...
export function generateAllSEORedirects(): RedirectMapping[] {
	...
	// Убираем дубликаты
	const uniqueRedirects = allRedirects.filter((redirect, index, self) => 
...
export function validateRedirects(redirects: RedirectMapping[]): {
	...
	if (duplicateSources.length > 0) {
		errors.push(`Duplicate source URLs found: ${[...new Set(duplicateSources)].join(', ')}`)
	}
	
	// Проверяем циклические редиректы
	for (const redirect of redirects) {
		if (redirect.source === redirect.destination) {
			errors.push(`Circular redirect detected: ${redirect.source} -> ${redirect.destination}`)
		}
	}
	...
	for (const redirect of redirects) {
		if (redirect.source.length > 100) {
			warnings.push(`Long source URL (${redirect.source.length} chars): ${redirect.source}`)
		}
	}
...
console.log(`🎯 High-frequency targets: "рассчитать" (4.1M) + "посчитать" (1.6M)`)
