...
export function useBMICalculator(initialValues?: Partial<BMIInput>) {
	...
	const calculateBMR = useCallback(
		(weight: number, height: number, age: number, gender: Gender): number => {
			if (gender === 'male') {
	...
	const calculate = useCallback(() => {
		...
		const bmiResult: BMIResult = {
			...
			idealWeight: {
				min:
					unitSystem === 'imperial'
						? idealWeightMin / 0.453592
						: idealWeightMin,
				max:
					unitSystem === 'imperial' ? idealWeightMax / 0.453592 : idealWeightMax
			},
			weightToLose:
				unitSystem === 'imperial' ? weightToLose / 0.453592 : weightToLose,
			weightToGain:
				unitSystem === 'imperial' ? weightToGain / 0.453592 : weightToGain,
			calories: {
		...
		if (waist && neck) {
			const waistCm =
				unitSystem === 'imperial' ? parseFloat(waist) * 2.54 : parseFloat(waist)
			const neckCm =
				unitSystem === 'imperial' ? parseFloat(neck) * 2.54 : parseFloat(neck)
			const hipCm = hip
				? unitSystem === 'imperial'
					? parseFloat(hip) * 2.54
					: parseFloat(hip)
	...
	const updateField = useCallback(
		(
			field: keyof BMIInput,
			value: string | Gender | ActivityLevel | UnitSystem
		) => {
	...
	const loadExample = useCallback(() => {
		const exampleData: Partial<BMIInput> =
