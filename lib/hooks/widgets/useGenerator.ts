...
export interface GeneratorConfig<T, TOptions extends GeneratorOptions = GeneratorOptions> {
...
export function useGenerator<T, TOptions extends GeneratorOptions = GeneratorOptions>(
  ...
  const generateBatch = useCallback(async (count: number) => {
    ...
    if (results.length > 0) {
      ...
      setHistory(prev => [...historyItems, ...prev].slice(0, config.maxHistory || 50))
  ...
  const updateOption = useCallback(<K extends keyof TOptions>(
    key: K,
    value: TOptions[K]
  ) => {
  ...
  const copyHistory = useCallback(() => {
    ...
    const text = history
      .map(item => config.format ? config.format(item.value) : String(item.value))
      .join('\n')
  ...
  // Add to favorites
  const addToFavorites = useCallback((value?: T) => {
    ...
    const exists = favorites.some(f => 
      JSON.stringify(f) === JSON.stringify(valueToAdd)
    )
