...
import { fontStyles, generateZalgoText, exampleTexts, type FontStyles } from '@/lib/data/fancy-text-data'
...
export function useFancyTextGenerator(options: UseFancyTextGeneratorOptions = {}) {
  ...
  const copyToClipboard = useCallback(async (text: string, styleKey: string) => {
    try {
      ...
      toast.success(translations?.copied || `${fontStyles[styleKey]?.name || 'Text'} copied to clipboard!`)
    } catch (err) {
