...
export function useCSSGradientGenerator(options: UseCSSGradientGeneratorOptions = {}) {
  ...
  const updateLinearDirection = useCallback((linearDirection: LinearDirection) => {
    updateSettings({ linearDirection })
  }, [updateSettings])

  const updateLinearAngle = useCallback((linearAngle: number) => {
    updateSettings({ linearAngle, linearDirection: 'custom' })
  }, [updateSettings])

  // Update radial gradient settings
  const updateRadialShape = useCallback((radialShape: RadialShape) => {
    updateSettings({ radialShape })
  }, [updateSettings])

  const updateRadialSize = useCallback((radialSize: RadialSize) => {
    updateSettings({ radialSize })
  }, [updateSettings])

  const updateRadialPosition = useCallback((radialPositionX: number, radialPositionY: number) => {
    updateSettings({ radialPositionX, radialPositionY })
  }, [updateSettings])

  // Update conic gradient settings
  const updateConicAngle = useCallback((conicAngle: number) => {
    updateSettings({ conicAngle })
  }, [updateSettings])

  const updateConicPosition = useCallback((conicPositionX: number, conicPositionY: number) => {
  ...
  const addColorStop = useCallback(() => {
    ...
    const newStops = [...settings.colorStops, newStop].sort((a, b) => a.position - b.position)
  ...
  const updateSelectedStop = useCallback((updates: Partial<ColorStop>) => {
    if (selectedStopId) {
  ...
  const applyPresetGradient = useCallback((preset: PresetGradient) => {
    ...
    toast.success(translations?.gradientApplied || `Applied "${preset.name}" gradient`)
  }, [translations])
  ...
  const generateRandom = useCallback(() => {
    ...
    toast.success(translations?.gradientRandomized || 'Random gradient generated')
  }, [translations])
  ...
  const exportGradient = useCallback((format: 'css' | 'scss' | 'tailwind' | 'json') => {
    let content = ''
    let filename = ''
