...
export function useCompoundInterestCalculator(
	initialValues?: Partial<CompoundInterestInput>
) {
	...
	const updateField = useCallback(
		<K extends keyof CompoundInterestInput>(
			field: K,
	...
	const calculate = useCallback(() => {
		...
		if (principal === 0 && monthlyContribution === 0) {
			toast.error(
				'Please enter either principal amount or monthly contribution'
			)
			return
		}
		...
		try {
			...
			const contributionPeriods =
				COMPOUNDING_FREQUENCIES[input.contributionFrequency]
			const contributionAmount =
				monthlyContribution * (12 / contributionPeriods)
				...
				if (month === 0) {
					...
					if (isYearEnd || month === years * 12) {
						const totalContributions =
							principal + contributionAmount * contributionPeriods * year
			...
			// Final calculations
			const totalContributions =
				principal + contributionAmount * contributionPeriods * years
			const totalInterest = balance - totalContributions
			const effectiveRate =
				principal > 0 ? (Math.pow(balance / principal, 1 / years) - 1) * 100 : 0
