...
export function usePercentageCalculator() {
  const [activeType, setActiveType] = useState<CalculationType>('percentOfNumber')
  ...
  const [results, setResults] = useState<Record<CalculationType, PercentageResult | null>>({
  ...
  // Copy result
  const copyResult = useCallback((type: CalculationType) => {
