...
export function useWidgetTips({ 
  ...
  const markTipAsCompleted = useCallback((tipId: string) => {
    setCompletedTips(prev => {
      if (!prev.includes(tipId)) {
        ...
        // Show progress toast
        const completionRate = Math.round((updated.length / tips.length) * 100)
  ...
  const completionRate = tips.length > 0 
    ? Math.round((completedTips.length / tips.length) * 100)
    : 0
...
export const calculatorTips: WidgetTip[] = [
  {
    ...
    description: 'The calculator automatically validates your input and shows helpful error messages',
    category: 'basic'
  },
  {
    id: 'keyboard-nav',
...
export const converterTips: WidgetTip[] = [
  {
    id: 'paste-input',
    title: 'Quick Paste',
    description: 'Use Ctrl/Cmd + V to paste content directly into the input field',
    category: 'shortcut'
  },
  {
    id: 'batch-convert',
    title: 'Batch Conversion',
    description: 'Convert multiple items at once by separating them with commas',
    category: 'advanced'
  },
  {
    id: 'format-options',
...
export const editorTips: WidgetTip[] = [
  {
    ...
    description: 'The editor automatically detects and highlights your code syntax',
    category: 'basic'
  },
  {
    id: 'auto-format',
