...
export const unitCategories: UnitCategory[] = [
	{
		...
		units: [
			...
			{
				// Ярд — ровно 0,9144 м по тому же соглашению 1959 года, что и
				// дюйм с футом. В быту встречается в гольфе, американском
				// футболе и рулонных тканях.
				id: 'yd',
				nameRu: 'ярды',
				symbol: 'ярд',
				toBase: v => v * 0.9144,
			...
			{
				// Средняя длина шага взрослого человека при ходьбе — величина
				// приблизительная (реально зависит от роста и темпа), для
				// расчёта взято общепринятое среднее 0,75 м, как в большинстве
				// шагомеров и фитнес-трекеров.
				id: 'step',
				nameRu: 'шаги',
				symbol: 'шаг',
				toBase: v => v * 0.75,
	...
	},
	{
		...
		units: [
			...
			{
				// Метрическая чайная ложка — 5 мл ровно, та же система мер,
				// что и столовая ложка ниже.
				id: 'tsp',
				nameRu: 'чайные ложки',
				symbol: 'ч. л.',
				toBase: v => v * 5,
	...
	},
	{
		...
		units: [
			...
			{
				// Миля в час — ровно 1609,344 / 3600 м/с, то же соглашение
				// 1959 года, что и сухопутная миля.
				id: 'mph',
				nameRu: 'мили в час',
				symbol: 'миля/ч',
				toBase: v => v * 0.44704,
	...
	},
	{
		...
		units: [
			...
			{
				// Бар — ровно 100 000 Па, поэтому 1 МПа это ровно 10 бар.
				id: 'bar',
				nameRu: 'бары',
				symbol: 'бар',
				toBase: v => v * 100000,
			...
			{
				// Физическая (нормальная) атмосфера — 101 325 Па ровно,
				// значение закреплено X ГКМВ в 1954 году.
				id: 'atm',
				nameRu: 'атмосферы',
				symbol: 'атм',
				toBase: v => v * 101325,
			...
			{
				// Техническая атмосфера — это 1 кгс/см², 98 066,5 Па. Именно
				// в ней градуированы советские манометры, и от физической
				// атмосферы она отличается примерно на 3%.
				id: 'at',
				nameRu: 'технические атмосферы',
				symbol: 'ат',
				toBase: v => v * 98066.5,
				fromBase: v => v / 98066.5
			},
			{
				// Миллиметр ртутного столба, он же торр: 101 325 / 760 Па,
				// то есть в физической атмосфере ровно 760 мм рт. ст.
				// Метрологический «конвенционный» мм рт. ст. чуть другой
				// (133,322387415 Па), но в медицине и технике считают по торру.
				id: 'mmhg',
				nameRu: 'миллиметры ртутного столба',
				symbol: 'мм рт. ст.',
				toBase: v => (v * 101325) / 760,
				fromBase: v => (v * 760) / 101325
			},
			{
				// Фунт на квадратный дюйм — давление в шинах и компрессорах
				// в американских источниках.
				id: 'psi',
				nameRu: 'фунты на кв. дюйм',
				symbol: 'psi',
