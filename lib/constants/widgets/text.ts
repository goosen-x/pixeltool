...
export const textWidgets: Widget[] = [
	...
	},
	{
		...
		// Не утверждён финально — см. Widget['demo'].
		demo: true,
		// Вордстат: «перевернуть текст» 8601/мес, но реальный чистый
		// generator-intent заметно меньше заголовка — большая часть разбора
		// («как перевернуть текст в ворде/экселе» 2843+2843+761+669+558) это
		// про сам офисный пакет (повернуть ориентацию текста в документе), не
		// про онлайн-тул. Чистый остаток — «вверх ногами»/«вверх» 553+503.
		searchVolume: 8601,
		icon: FlipVertical,
		category: 'text',
		translationKey: 'flipText',
		path: 'flip-text',
		gradient: 'from-teal-500 to-cyan-600',
		title: 'Перевернуть текст',
		description:
			'Переверните текст вверх ногами через юникод-глифы или просто прочитайте его задом наперёд.',
		useCase:
			'Написать текст вверх ногами для шутки в соцсетях или просто прочитать слово задом наперёд',
		recommendedTools: [
			'fancy-text-generator',
			'text-case-converter',
			'special-symbols-picker'
		...
		difficulty: 'beginner',
		tags: ['flip', 'upside-down', 'reverse', 'text', 'unicode', 'mirror'],
		metaTitle: 'Перевернуть текст онлайн — вверх ногами или задом наперёд',
		updatedAt: '2026-08-16',
		metaDescription:
			'Переверните текст онлайн: вверх ногами через юникод-глифы (uƃᴉs ǝʞᴉl sᴉɥʇ) или просто задом наперёд. Копирование в один клик.',
		faqs: [
			{
				question: 'В чём разница между «вверх ногами» и «задом наперёд»?',
				answer:
					'«Вверх ногами» заменяет буквы на юникод-символы, которые выглядят как та же буква, повёрнутая на 180°, — и меняет порядок символов на обратный, как будто текст реально перевернули. «Задом наперёд» просто меняет порядок символов, без замены глифов — буквы остаются обычными, но идут в обратном порядке.'
			},
			{
				question: 'Почему русские буквы не переворачиваются?',
				answer:
					'У латинского алфавита в юникоде исторически набралось много символов, которые случайно похожи на перевёрнутые буквы (в основном из африканских и фонетических алфавитов). У кириллицы таких совпадений почти нет, поэтому русские буквы остаются как есть — меняется только порядок символов.'
			},
			{
				question: 'Перевёрнутый текст будет читаться везде одинаково?',
				answer:
					'В большинстве случаев да — это обычные символы юникода, не картинка. Но отображение зависит от шрифта конкретного приложения: где-то редкие символы могут показываться квадратиками, если в системе нет подходящего шрифта.'
			}
		]
	}
]
