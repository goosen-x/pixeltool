...
async function fixBase64Encoder() {
  ...
  const filePath = path.join(process.cwd(), 'app/[locale]/(tools)/tools/base64-encoder/page.tsx')
  ...
  content = content.replace(/          <\/WidgetGridCard>/g, '          </WidgetSection>')
  content = content.replace(/            <\/WidgetGridCard>/g, '            </WidgetOutput>')
