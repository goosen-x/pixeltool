...
async function checkAnalyticsSetup() {
	...
	try {
		...
		if (error) {
			...
			// Clean up test event
			await supabase.from('usage_events').delete().eq('id', data.id)
		}
	} catch (error) {
	...
	try {
		...
		if (error) {
			...
			if (count === 0) {
				console.log(
					'   → Run: npx tsx lib/scripts/populate-widgets-supabase.ts'
				)
			}
		}
	} catch (error) {
	...
	try {
		const response = await fetch(
			'http://localhost:3000/api/analytics/stats/age-calculator'
		)
		if (response.ok) {
			...
			console.log(
				'⚠️  API endpoint returned error (server might not be running)'
			)
		}
	} catch (error) {
