...
async function populateWidgets() {
	...
	const { data: existingWidgets } = await supabase.from('widgets').select('id')
	...
	for (const widget of widgets) {
		try {
			...
			const { error: widgetError } = await supabase.from('widgets').insert({
				...
				slug: widget.path,
				icon: widget.icon
					? widget.icon.name || widget.icon.displayName || widget.id
					: null,
			...
			if (widgetError) {
				console.error(
					`❌ Failed to insert widget ${widget.id}:`,
					widgetError.message
				)
			...
			const translations = [
				{
					...
					description:
						enMessages.widgets[widget.translationKey]?.description || ''
				},
				{
					widget_id: widget.id,
					locale: 'ru',
					...
					description:
						ruMessages.widgets[widget.translationKey]?.description || ''
				}
			]
			...
			if (translationError) {
				console.error(
					`❌ Failed to insert translations for ${widget.id}:`,
			...
			if (faqs.length > 0) {
				...
				if (faqError) {
					console.error(
						`⚠️  Failed to insert FAQs for ${widget.id}:`,
						faqError.message
					)
				}
			}
			...
			if (widget.tags && widget.tags.length > 0) {
				...
				if (tagError) {
					console.error(
						`⚠️  Failed to insert tags for ${widget.id}:`,
						tagError.message
					)
				}
			}
	...
	for (const widget of widgets) {
		if (widget.recommendedTools && widget.recommendedTools.length > 0) {
			try {
				...
				if (relatedError) {
					console.error(
						`❌ Failed to insert relations for ${widget.id}:`,
