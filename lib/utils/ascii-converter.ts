...
export type AsciiFont = 'standard' | 'block'
...
export function textToAscii(
	...
	const fonts: Record<string, Record<string, string[]>> = {
		standard: {
			...
			// Кириллица описана только здесь — «Крупный» получает её
			// автоматически, потому что собирается из этих же глифов (см.
			// fonts.block ниже). Одиннадцать букв по форме совпадают с
			// латинскими (А=A, В=B, Е=E, К=K, М=M, Н=H, О=O, Р=P, С=C, Т=T,
			// Х=X) — переиспользуем те же контуры, только с кириллической
			// буквой в качестве «чернил».
