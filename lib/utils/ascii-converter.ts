...
export function textToAscii(
	...
	for (let i = 0; i < lineCount; i++) {
		...
		for (const char of upperText) {
			// «Мелкий» шрифт рисует только латиницу — у кириллицы, цифр и
			// пунктуации родного глифа нет. Раньше в этом случае брали верхние
			// `lineCount` строк из 5-строчного глифа «Стандартного»: верхушка
			// буквы без низа — это обрубок, а не буква (баг «мелкий шрифт не
			// работает» на кириллице). Вместо обрубка — сам символ первой
			// строкой, остальные пустые: не так стилизованно, как родной
			// мелкий глиф, но хотя бы читается.
			const charPattern =
				selectedFont[char] ||
				(font === 'small'
					? Array.from({ length: lineCount }, (_, row) =>
							row === 0 ? char : ' '.repeat(char.length)
						)
					: fonts.standard[char]) ||
				selectedFont[' '] ||
				fonts.standard[' ']
