...
// Get all widgets
export async function getWidgets(locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget[]> {
...
// Get widget by ID
export async function getWidgetById(id: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget | null> {
  const staticData = staticWidgets.getStaticWidgetById(id, locale)
  if (!staticData) return null
  
  const isDbReady = await checkWidgetsDbReady()
  if (isDbReady) {
    const dbFunction = () => supabaseWidgets.getWidgetById(id, locale).then(data => data || staticData)
...
// Get widget by slug
export async function getWidgetBySlug(slug: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget | null> {
  const staticData = staticWidgets.getStaticWidgetBySlug(slug, locale)
  if (!staticData) return null
  
  const isDbReady = await checkWidgetsDbReady()
  if (isDbReady) {
    const dbFunction = () => supabaseWidgets.getWidgetBySlug(slug, locale).then(data => data || staticData)
...
export async function getWidgetsByCategory(category: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget[]> {
  const staticData = staticWidgets.getStaticWidgetsByCategory(category, locale)
  
  const isDbReady = await checkWidgetsDbReady()
  if (isDbReady) {
    const dbFunction = () => supabaseWidgets.getWidgetsByCategory(category, locale)
...
// Get widget FAQs
export async function getWidgetFAQs(widgetId: string, locale: 'en' | 'ru' = 'en'): Promise<{ question: string; answer: string }[]> {
...
// Get popular widgets
export async function getPopularWidgets(locale: 'en' | 'ru' = 'en', limit: number = 6): Promise<LocalizedWidget[]> {
...
// Get new widgets
export async function getNewWidgets(locale: 'en' | 'ru' = 'en', limit: number = 6): Promise<LocalizedWidget[]> {
...
// Get widgets by tag
export async function getWidgetsByTag(tag: string, locale: 'en' | 'ru' = 'en'): Promise<LocalizedWidget[]> {
