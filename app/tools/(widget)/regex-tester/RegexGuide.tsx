...
const SYNTAX: { name: string; desc: string }[] = [
	...
	{ name: 'a|b', desc: 'Альтернатива, то есть a или b' }
]
	...
	return (
		<section className='mt-12 max-w-3xl mx-auto text-left text-foreground'>
			...
			<p className='mt-3 leading-relaxed'>
				...
				URL) и замены текста. Все три задачи умеет тестер выше.
			</p>
			...
			<p className='mt-3 leading-relaxed'>
				По умолчанию квантификаторы «жадные», они захватывают максимально
				длинный кусок текста. Добавленный после них знак вопроса делает их
				«ленивыми», и тогда они останавливаются на первом же подходящем
				совпадении.
			</p>
			<GuideCodeBlock
				className='mt-4'
...
text.match(/<b>.*?<\\/b>/)  // ленивый: остановится на первом же </b>`}
			...
			<p className='mt-3 leading-relaxed'>
				Круглые скобки <Code>(...)</Code> группируют часть шаблона и заодно
				запоминают найденный фрагмент. Его можно достать из результата или
				подставить при замене как <Code>$1</Code>, <Code>$2</Code> и так далее.
				Если запоминать фрагмент не нужно, скажем группа стоит только ради
				альтернативы <Code>a|b</Code> внутри более крупного шаблона, берите
				группу без захвата <Code>(?:...)</Code>. Она не засоряет список
				совпадений.
			</p>
			<GuideCodeBlock
				className='mt-4'
...
match.groups.day    // '15'`}
			...
			<p className='mt-3 leading-relaxed'>
				...
				но обращаться к результату можно по имени, а не по номеру. Когда групп
				больше двух-трёх, код так читается заметно проще.
			</p>
...
(?<!...)  // позади не должно быть`}
			...
			<p className='mt-3 leading-relaxed'>
				...
				Python). В тестере выше их включают переключателями.
			</p>
			<div className='mt-4 overflow-x-auto'>
				<table className='w-full border-collapse text-sm'>
					<thead>
			...
			<p className='mt-3 leading-relaxed'>
				...
				строкой, а флаги отдельным аргументом функции. Именованные группы Python
				исторически писал как <Code>(?P&lt;name&gt;...)</Code>, но синтаксис{' '}
				<Code>(?&lt;name&gt;...)</Code> из тестера тоже валиден. Переключатель
				языка наверху страницы меняет именно эту обвязку в кнопке «Копировать
				код», сам паттерн остаётся общим.
			</p>

			<p className='mt-8 leading-relaxed'>
				Короткая версия этой шпаргалки с живым тестером есть в статье{' '}
				<Link
