...
const SYNTAX: { name: string; desc: string }[] = [
	...
	{ name: '[abc] [^abc]', desc: 'Один символ из набора и один символ не из набора' },
...
const FLAGS: { name: string; desc: string }[] = [
	...
	{ name: 'm', desc: '^ и $ работают на границах каждой строки, а не только всего текста' },
	{ name: 's', desc: 'Точка . дополнительно совпадает с переносом строки' },
	{ name: 'u', desc: 'Корректная работа с Unicode — многобайтовыми символами и эмодзи' }
]
	...
	return (
		<section className='mt-12 max-w-3xl mx-auto text-left text-foreground'>
			...
			<p className='mt-3 leading-relaxed'>
				...
				<Code>^\d{'{3}'}-\d{'{4}'}$</Code> находит любую строку вида «123-4567»,
				не зная заранее конкретных цифр. Regex одинаково используют для поиска,
				проверки формата (email, телефон, URL) и замены текста — три задачи,
				которые умеет тестер выше.
			</p>
			...
			<ol className='mt-3 space-y-2 leading-relaxed'>
				<li>Введите шаблон в поле «Регулярное выражение» — без слэшей по краям.</li>
				<li>Вставьте текст для проверки — совпадения подсветятся сразу по мере ввода.</li>
				<li>Включайте флаги под полем шаблона, чтобы поменять поведение поиска.</li>
				<li>
...
text.match(/<b>.*?<\\/b>/)  // ленивый: остановится на первом же </b>`}
			...
			<p className='mt-3 leading-relaxed'>
				Круглые скобки <Code>(...)</Code> не только группируют часть шаблона,
				но и запоминают найденный фрагмент — его можно достать из результата
				или подставить при замене как <Code>$1</Code>, <Code>$2</Code> и так
				далее. Если запоминать фрагмент не нужно — например, группа нужна
				только для альтернативы <Code>a|b</Code> внутри более крупного
				шаблона — используйте некапturing-группу <Code>(?:...)</Code>: она не
				засоряет список совпадений.
			</p>
			<GuideCodeBlock
				className='mt-4'
...
(?<!...)  // позади не должно быть`}
			...
			<p className='mt-3 leading-relaxed'>
				...
				<Code>{`(?<=\\$)\\d+(\\.\\d{2})?`}</Code> находит «49.99» в строке «$49.99»,
				не захватывая сам знак доллара.
			</p>
			...
			<p className='mt-3 leading-relaxed'>
				Ядро синтаксиса — классы символов, квантификаторы, группы — одинаково
				во всех трёх языках. Отличается обвязка: JavaScript пишет шаблон между
				слэшами (<Code>/regex/флаги</Code>), PHP оборачивает его в
				дополнительный разделитель-кавычку (<Code>&apos;/regex/флаги&apos;</Code>),
				а Python передаёт шаблон обычной строкой и флаги — отдельным
				аргументом функции. Именованные группы Python исторически писал как{' '}
				<Code>(?P&lt;name&gt;...)</Code>, но синтаксис <Code>(?&lt;name&gt;...)</Code>{' '}
				из тестера тоже валиден. Переключатель языка наверху страницы меняет
				именно эту обвязку в кнопке «Копировать код» — сам паттерн остаётся
				общим.
			</p>
