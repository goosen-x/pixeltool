...
const VERSIONS: [string, string, string][] = [
	...
	[
		'nil',
		'Только нули',
		'Заглушка «идентификатора нет» — удобнее, чем NULL, когда колонка обязательная'
	]
]
		...
		<div className='mx-auto mt-16 max-w-3xl space-y-12'>
			...
			<section>
				...
				<ul className='mt-4 space-y-2 text-muted-foreground'>
					<li>
						Первый символ третьей группы — <strong>версия</strong>. Здесь{' '}
					...
					<li>
						Первый символ четвёртой группы — <strong>вариант</strong>. Значения{' '}
						<code className='rounded bg-secondary px-1.5 py-0.5 font-mono text-xs'>
							8
						</code>
						,{' '}
			...
			<section>
				...
				<p className='mt-3 text-muted-foreground'>
					Уникальность — это единственное, что гарантирует формат. Из этого
					следуют три ограничения, о которые спотыкаются регулярно.
				</p>
				<ul className='mt-4 space-y-2 text-muted-foreground'>
					<li>
						<strong className='text-foreground'>Это не секрет.</strong> UUID v4
						нельзя угадать, но он попадает в адресную строку, историю браузера,
						логи и заголовок{' '}
						<code className='rounded bg-secondary px-1.5 py-0.5 font-mono text-xs'>
							Referer
						</code>
					...
					<li>
						<strong className='text-foreground'>
							v1 и v7 раскрывают время.
						</strong>{' '}
						Из такого идентификатора извлекается момент создания записи с
						точностью до миллисекунд, а v1 выдаёт ещё и сетевой адрес машины.
						Там, где порядок и время регистрации не должны быть видны снаружи,
						берите v4.
					</li>
					<li>
						<strong className='text-foreground'>
							Он не защищает от дублей.
						</strong>{' '}
