...
const TABLE: { px: number; rem: string; usage: string }[] = [
	{ px: 4, rem: '0.25rem', usage: 'Мелкий отступ, зазор между иконкой и текстом' },
...
rem = px  ÷ 16      /* 24px  ÷ 16 = 1.5rem */`}</code>
			...
			<p className='mt-3 leading-relaxed'>
				<Code>rem</Code> всегда считается от корневого элемента, а <Code>em</Code>{' '}
				— от размера шрифта родителя. Разница вылезает при вложенности:{' '}
				<Code>em</Code> накапливается.
			</p>
			<pre className='mt-4 overflow-x-auto rounded-lg bg-secondary p-4 text-sm'>
...
.a, .a .b, .a .b .c { font-size: 0.9rem; }  /* везде 14.4px */`}</code>
			...
			<p className='mt-3 leading-relaxed'>
				...
				зависеть от текста рядом: например, внутренний отступ кнопки, который сам
				подстраивается под кегль надписи.
			</p>
