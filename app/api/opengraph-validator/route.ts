	...
	try {
		...
		if (error instanceof Error) {
			...
			if (error.message.includes('fetch failed')) {
				...
				// висят именно так, при этом x.com/facebook.com/vk.com/
				// habr.com с того же сервера открываются нормально — это не
				// общая проблема сети, а блокировка конкретных площадок
				// против скрейпинга по IP хостинга (частая практика для
				// видео/фото/мессенджеров). Список таких площадок будет
				// расти сам по себе, поэтому не перечисляем домены — просто
				// честно объясняем природу ошибки по коду, а не советуем
				// повторить попытку, которая тут никогда не поможет.
