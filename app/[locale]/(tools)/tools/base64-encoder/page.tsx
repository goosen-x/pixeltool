			...
			if (mode === 'encode') {
				...
				reader.onload = event => {
					...
					setStats({
						...
						compressionRatio:
							(new Blob([base64Data]).size / selectedFile.size - 1) * 100
					})
				...
				reader.onload = event => {
					...
					try {
						...
						setStats({
							...
							compressionRatio:
								(1 - new Blob([decoded]).size / selectedFile.size) * 100
						})
					} catch (error) {
	...
	const shortcuts = [
		{
			...
			description: t('shortcuts.process'),
			enabled: !!input && dataType === 'text'
		},
		{
			key: 'c',
			ctrl: true,
		...
		},
		{
			key: 's',
			ctrl: true,
			action: swapInputOutput,
			description: t('shortcuts.swap'),
			enabled: !!output
		},
		{
			key: 'k',
		...
		<WidgetLayout>
						...
						<div className='flex items-center gap-6'>
							...
							<div className='flex items-center space-x-2'>
								...
								<Label htmlFor='line-breaks'>
									Разбивать на строки (76 символов)
								</Label>
							</div>
						</div>
					)}
			...
			<div className='grid lg:grid-cols-2 gap-6'>
				...
				<div className='space-y-4'>
					...
					{/* Actions */}
					<WidgetSection icon={<Zap className='h-5 w-5' />} title='Действия'>
						<div className='space-y-2'>
							...
							>
								...
								Поменять местами и{' '}
								{mode === 'encode' ? 'декодировать' : 'кодировать'}
							</Button>
			...
			<div className='grid lg:grid-cols-3 gap-6'>
				...
				<div className='space-y-6'>
					<WidgetSection icon={<Info className='h-5 w-5' />} title='Информация'>
						<h3 className='font-semibold mb-4 flex items-center gap-2'>
							<Info className='w-4 h-4' />О Base64
						</h3>
						<div className='space-y-3 text-sm text-muted-foreground'>
							<p>
								Base64 — схема кодирования, представляющая двоичные данные в
								ASCII формате.
							</p>
							<div>
