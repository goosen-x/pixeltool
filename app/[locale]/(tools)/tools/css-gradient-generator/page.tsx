...
export default function CSSGradientGeneratorPage() {
  ...
  return (
    <WidgetLayout>
      {/* Preview Section */}
      <WidgetSection icon={Palette} title={t('sections.preview')} className="w-full">
        ...
        <WidgetSection icon={Sliders} title={t('sections.controls')}>
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <Label className="text-sm font-medium">{t('controls.gradientType')}</Label>
              <div className="grid grid-cols-3 gap-2">
                {(['linear', 'radial', 'conic'] as const).map((type) => (
                  <Button
                    ...
                    className={cn(
                      "capitalize",
                      settings.type === type && "bg-gradient-to-r from-primary to-accent text-white"
                    )}
                  >
                    {type}
                  </Button>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-muted/50">
                <Label htmlFor="repeating" className="text-sm">{t('controls.repeating')}</Label>
                <Button
                  id="repeating"
                  ...
                  size="sm"
                  className={cn(
                    settings.repeating && "bg-gradient-to-r from-primary to-accent text-white"
                  )}
                >
            ...
            {settings.type === 'linear' && (
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(['to top', 'to right', 'to bottom', 'to left', 'to top right', 'to bottom right'] as const).map((dir) => (
                      <Button
                        ...
                        variant={settings.linearDirection === dir ? 'default' : 'outline'}
                        size="sm"
                        className={cn(
                          "text-xs",
                ...
                <WidgetInput label={`${t('controls.angle')}: ${settings.linearAngle}°`}>
                  <Slider
            ...
            {settings.type === 'radial' && (
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['circle', 'ellipse'] as const).map((shape) => (
                      <Button
                        ...
                        variant={settings.radialShape === shape ? 'default' : 'outline'}
                        size="sm"
                        className={cn(
                          "capitalize",
                          settings.radialShape === shape && "bg-gradient-to-r from-primary to-accent text-white"
                        )}
                      >
                        {shape}
                      </Button>
                ...
                <WidgetInput label={t('controls.size')}>
                  <Select value={settings.radialSize} onValueChange={(value) => updateRadialSize(value as any)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <WidgetInput label={`X: ${settings.radialPositionX}%`}>
                    <Slider
                      ...
                      onValueChange={([value]) => updateRadialPosition(value, settings.radialPositionY)}
                      min={0}
                      max={100}
                      step={1}
                    />
                  ...
                  <WidgetInput label={`Y: ${settings.radialPositionY}%`}>
                    <Slider
                      ...
                      onValueChange={([value]) => updateRadialPosition(settings.radialPositionX, value)}
                      min={0}
                      max={100}
                      step={1}
                    />
            ...
            {settings.type === 'conic' && (
              <div className="space-y-4">
                <WidgetInput label={`${t('controls.angle')}: ${settings.conicAngle}°`}>
                  <Slider
                ...
                <div className="grid grid-cols-2 gap-4">
                  <WidgetInput label={`X: ${settings.conicPositionX}%`}>
                    <Slider
                      ...
                      onValueChange={([value]) => updateConicPosition(value, settings.conicPositionY)}
                      min={0}
                      max={100}
                      step={1}
                    />
                  ...
                  <WidgetInput label={`Y: ${settings.conicPositionY}%`}>
                    <Slider
                      ...
                      onValueChange={([value]) => updateConicPosition(settings.conicPositionX, value)}
                      min={0}
                      max={100}
                      step={1}
                    />
          ...
          <div className="space-y-4">
            ...
            {selectedStop && (
              <div className="space-y-4 pt-4 border-t">
                ...
                <WidgetInput label={`${t('controls.position')}: ${selectedStop.position}%`}>
                  <Slider
                    value={[selectedStop.position]}
                    onValueChange={([value]) => updateSelectedStop({ position: value })}
                    min={0}
                    max={100}
                    step={1}
                  />
                ...
                <WidgetInput label={`${t('controls.opacity')}: ${selectedStop.opacity}%`}>
                  <Slider
                    value={[selectedStop.opacity]}
                    onValueChange={([value]) => updateSelectedStop({ opacity: value })}
                    min={0}
                    max={100}
                    step={1}
                  />
      ...
      {/* Presets Section */}
      <WidgetSection icon={Sparkles} title={t('sections.presets')} className="mt-6">
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={activeCategory} onValueChange={setActiveCategory}>
              ...
              <SelectContent>
                <SelectItem value="all">{t('presets.allCategories')}</SelectItem>
