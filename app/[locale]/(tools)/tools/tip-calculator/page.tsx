              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {tipPresets.map((preset) => (
                    <Button
                      key={preset}
                      variant={tipPercent === preset && !customTipPercent ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => {
                ...
                <div className="space-y-2">
                  <Slider
                    value={[customTipPercent ? parseFloat(customTipPercent) : tipPercent]}
                    onValueChange={(value) => {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                <div className="text-sm text-muted-foreground">{t('totalWithTip')}</div>
                ...
                <>
                  <div className="border-t pt-4">
                    ...
                    <div className="flex justify-between items-center p-3 bg-muted/30 rounded-lg mb-2">
                      ...
                      <span className="font-semibold text-lg">${result.tipPerPerson}</span>
                    </div>

                    {/* Total per Person */}
                    <div className="flex justify-between items-center p-3 bg-muted/30 rounded-lg">
                      <span className="text-sm">{t('totalPerPerson')}</span>
                      <span className="font-semibold text-lg">${result.totalPerPerson}</span>
                    </div>
                  </div>
                </>
              )}
              ...
              <div className="border-t pt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('originalBill')}</span>
          ...
          <CardContent>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              <div className="p-3 bg-muted/30 rounded-lg">
                ...
                <div className="text-sm text-muted-foreground">{t('tipGuide.basic')}</div>
              </div>
              <div className="p-3 bg-muted/30 rounded-lg">
                <div className="font-semibold">15-20%</div>
                <div className="text-sm text-muted-foreground">{t('tipGuide.good')}</div>
              </div>
              <div className="p-3 bg-muted/30 rounded-lg">
                <div className="font-semibold">20%+</div>
                <div className="text-sm text-muted-foreground">{t('tipGuide.excellent')}</div>
              </div>
            </div>
          </CardContent>
        </Card>
