...
class TranslationTypeGenerator {
			...
			case 'object':
				if (Array.isArray(value)) {
					...
					const itemTypes = new Set(
						value.map(item => this.generateTypeForValue(item, key, level))
					)
					if (itemTypes.size === 1) {
				...
				for (const [objKey, objValue] of Object.entries(value)) {
					const valueType = this.generateTypeForValue(
						objValue,
						objKey,
						level + 1
					)
					...
					lines.push(
						`${this.indent(level + 1)}${formattedKey}${isOptional ? '?' : ''}: ${valueType}`
					)
				}
	...
	public generate() {
		...
		const widgetNames = Object.keys(this.enTranslations.widgets || {}).filter(
			key =>
				![
					'search',
					'favorites',
		...
		this.output.push(
			`export type GeneratedWidgetName = ${widgetNames.map(name => `'${name}'`).join(' | ') || 'never'}`
		)
		this.output.push('')
		...
		const outputPath = path.join(
			__dirname,
			'../types/generated-translations.ts'
		)
