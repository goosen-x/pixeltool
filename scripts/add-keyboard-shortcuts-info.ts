...
async function addKeyboardShortcutInfo() {
  ...
  for (const widgetDir of widgetDirs) {
    ...
    if (!content.includes("import { KeyboardShortcutInfo } from '@/components/widgets'")) {
    ...
    if (!inserted) {
      console.log(`⚠️  ${widgetDir} - couldn't find suitable location for KeyboardShortcutInfo`)
      continue
    }
