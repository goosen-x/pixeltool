...
class TranslationValidator {
	...
	private validateWidgetStructure(
		widget: any,
		widgetName: string,
		file: string
	): boolean {
		...
		for (const key of ruKeys) {
			if (!enKeys.has(key)) {
				this.addWarning(
					'ru.json',
					`widgets.${widgetName}.${key}`,
					`Extra translation key not in English: ${key}`
				)
			}
		}
	}
	...
	public validate() {
		...
		for (const widgetName of enWidgetNames) {
			...
			const enValid = this.validateWidgetStructure(
				enWidget,
				widgetName,
				'en.json'
			)
			const ruValid = this.validateWidgetStructure(
				ruWidget,
				widgetName,
				'ru.json'
			)
			...
			if (enValid && ruValid) {
				const hasExtraFields =
					Object.keys(enWidget).length > 3 || Object.keys(ruWidget).length > 3
				if (hasExtraFields) {
		...
		for (const widgetName of ruWidgetNames) {
			if (!enWidgetNames.includes(widgetName)) {
				this.addWarning(
					'ru.json',
					`widgets.${widgetName}`,
					'Extra widget not in English version'
				)
			}
		}
		...
		const countWidgetOccurrences = (obj: any, currentPath: string = '') => {
			for (const [key, value] of Object.entries(obj)) {
				if (
					enWidgetNames.includes(key) &&
					typeof value === 'object' &&
					'title' in (value as any)
				) {
		...
		for (const [widgetName, count] of widgetOccurrences) {
			if (count > 2) {
				this.addError(
					'en.json',
					widgetName,
					`Widget defined ${count} times (should be at most 2: brief + full)`
				)
			}
		}
