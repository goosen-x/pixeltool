...
function replaceTranslations(content, widgetId) {
	...
	content = content.replace(
		/\s*const\s+t\s*=\s*useTranslations\([^)]+\)\s*\n?/g,
		''
	)
	...
	content = content.replace(tCallRegex, (match, key) => {
		...
		for (const k of keys) {
			if (value && typeof value === 'object' && k in value) {
				...
				console.log(
					`Warning: Translation key not found: ${key} in widget ${widgetId}`
				)
		...
		if (typeof value === 'object') {
			console.log(
				`Warning: Complex translation object for key: ${key} in widget ${widgetId}`
			)
			return `'${key}'` // Return the key as fallback
		}
