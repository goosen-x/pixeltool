 ...
 *
 * ⚠️ ПЛАТНО. В отличие от Wordstat (бесплатная квота), Yandex Search API v2
 * списывает деньги с того же баланса за каждый запрос. Кэша нет — каждый
 * запуск --built бьёт по API заново (~285 запросов на 116 фраз, т.к. для
 * тулов вне топ-30 листаются все 3 страницы). Не гонять без необходимости
 * и не перезапускать один и тот же набор фраз ради разных форматов вывода —
 * 05.09.2026 три подряд прогона --built снесли 500₽ баланса за один сеанс.
 */
