...
Есть одна деталь, о которой стоит знать: **элемент не может запрашивать сам
себя.** Задав контейнер, вы обращаетесь к нему изнутри — из дочерних элементов.
...
Поэтому под контейнерные запросы почти всегда заводят лишнюю обёртку — это не
костыль, а норма.

Вторая деталь: `container-type: inline-size` делает элемент **источником
containment по инлайновой оси**. Его высота по-прежнему зависит от содержимого,
а вот ширина больше не подстраивается под детей — иногда это меняет раскладку
