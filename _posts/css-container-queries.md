...
вьюпорт-единицами. Они не заменяют `rem`, а дополняют его: размеры шрифта
по-прежнему удобнее держать в `rem`, чтобы они масштабировались вместе с
настройками пользователя, а в `cqw` переводить то, что должно тянуться за
шириной контейнера. Перевести исходные пиксели в `rem` помогает
[конвертер px в rem](/tools/px-rem-converter).
