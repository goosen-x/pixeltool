...
1. Держите единые префиксы в именах: `--color-`, `--space-`, `--font-`.
2. Для стилей, без которых страница развалится, указывайте резервное значение
   вторым аргументом `var()`.
3. Комментируйте вычисления, через месяц `calc()` из трёх переменных читается
   плохо.
4. Группируйте связанные переменные рядом, а не по алфавиту.
5. Системные переменные объявляйте в `:root`, локальные — там, где они нужны.
