...
случайный набор символов, пока не знаешь, что каждый символ в нём отдельная
команда с чётким смыслом. Разберём алфавит regex по токенам, а дальше в этой же
статье стоит [живой тестер](/tools/regex-tester), чтобы сразу проверить свой
пример, не открывая другую вкладку.
...
По умолчанию квантификаторы жадные, они забирают максимум возможного:
...
Жадный `.+` не останавливается на первой попавшейся `>`. Он пытается забрать как
можно больше символов, а потом «сдаёт назад» ровно настолько, чтобы совпадение
всё ещё нашлось. Добавьте `?` сразу после квантификатора, чтобы сделать его
ленивым, и он остановится при первой возможности:
...
Правило простое. Нужен самый короткий фрагмент между двумя разделителями, берите
ленивый квантификатор (`*?`, `+?`, `??`). Нужен самый длинный, оставляйте жадный
по умолчанию.
...
Без `g` `String.prototype.match()` вернёт только первое совпадение и
остановится. Это частая причина, почему «регулярка вроде рабочая, а находит
только один результат».

## Готовые паттерны

Выражения, которые пригождаются чаще всего. На экзотических случаях они не
идеальны (RFC для email длиннее любой статьи), но покрывают подавляющее
...
// Латиница внутри кириллического слова: a, c, e, o, p, x и другие двойники,
// из-за которых поиск по тексту не находит нужное
const mixedAlphabetPattern = /[а-яё]+[a-zA-Z]|[a-zA-Z][а-яё]+/
```

Последний случай встречается чаще, чем кажется: латинская `a` внутри русского
слова невидима глазом, но ломает поиск, сортировку и сравнение строк. Если нужно
не составлять выражение, а найти такие места в готовом тексте, есть
[проверка на латиницу в кириллице](/tools/latin-cyrillic-checker) — она
подсвечивает буквы-двойники прямо в тексте.
...
Забыли экранировать спецсимвол. Точка `.` в regex значит «любой символ», а не
буквальную точку. Чтобы найти именно точку, экранируйте её: `\.`. То же с `+`,
`*`, `?`, `(`, `)`, `[`, `]`, все они спецсимволы, и для буквального совпадения
нужен обратный слэш перед ними.

Забыли `^` и `$`. Без них выражение ищет совпадение где угодно в строке, а не
проверяет строку целиком. `\d{3}` совпадёт и внутри `abc123def`. Если нужна
проверка «строка целиком состоит из трёх цифр», ставьте оба якоря: `^\d{3}$`.

Перепутали `[]` и `()`. Квадратные скобки означают «один из перечисленных
символов», `[abc]` совпадёт с одной буквой `a`, `b` или `c`. Круглые скобки
задают группу, `(abc)` совпадёт только с точной последовательностью «abc»
целиком.

## Коротко

- Регулярное выражение читается токен за токеном. `\d`, `\w`, `\s` — классы
  символов, `*`, `+`, `?`, `{n,m}` — сколько раз, `^` и `$` — границы строки.
...
- `.` нужно экранировать (`\.`), если имеется в виду буквальная точка, а не
  «любой символ».
- Готовые паттерны в голове держать необязательно, проще проверить их на живых
  данных.
