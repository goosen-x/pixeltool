...
размера. Браузер открывает файл, смотрит, что ему доступно, и берёт подходящий
кадр: 16 пикселей для вкладки, 32 — для экрана с высокой плотностью, 48 — для
ярлыка в Windows.
...
одном файле — вы не получаете. [Генератор фавиконов](/tools/favicon-generator)
...
- **Максимальный контраст.** Иконка живёт на фоне вкладки, а он бывает и
  светлым, и тёмным. Бледные тона исчезают.
- **Никаких тонких линий.** Штрих в один пиксель при масштабировании пропадёт
  или замылится.
