...
excerpt:
  ...
  кнопок и наведения.'
coverImage: '/images/blog/css-shadows-2.png'
date: '2026-07-13T10:00:00.000Z'
author:
  name: Дмитрий Борисенко
  picture: '/images/avatar.jpeg'
ogImage:
  url: '/images/blog/css-shadows-2.png'
related:
  - css-animations
  - css-variables
  - css-grid-layout
...
Обратите внимание, тень здесь синяя, того же тона, что и сама кнопка. Так она
выглядит частью элемента, а не грязным пятном под ним.
...
Тень текста поверх фотографии решает вопрос читаемости. Но контраст она не
заменяет. Если светлый текст лежит на светлом участке снимка, тень лишь
...
Крупное размытие. 15–30px вместо 3–5px. Резкая тень читается как ошибка вёрстки,
а не как объём.

Оттенок вместо черноты. Тень под синей кнопкой должна быть синеватой, под тёплой
карточкой коричневатой. Проще всего взять цвет фона и затемнить его. Крутить
светлоту удобно в HSL, где она вынесена в отдельный канал:
...
собирают многослойную глубину, из этого же сделаны тени в популярных
дизайн-системах.
...
`border-radius`. Кстати, если внутри скруглённой карточки лежит ещё одна, их
радиусы нельзя брать одинаковыми — дуги разойдутся. Правильный внутренний радиус
считается как внешний минус отступ, и для этого есть
[калькулятор вложенных скруглений](/tools/nested-border-radius).
...
- Для иконок с прозрачностью берут `filter: drop-shadow()`, а не `box-shadow`.
- Анимируйте `opacity` псевдоэлемента, а не саму тень.
