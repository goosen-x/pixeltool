...
Подбирать четыре числа `cubic-bezier` вслепую — мучение. Тяните контрольные
точки мышью и сразу смотрите, как меняется движение:
...
Собрать свои `@keyframes` ползунками и посмотреть результат вживую:
...
1. **Соблюдайте меру**: анимации должны дополнять интерфейс, а не отвлекать от
   него
2. **Уважайте предпочтения по движению**:
   ...
   ```
3. **Проверяйте производительность**: анимации бьют по метрике TBT, а она весит
   30% в оценке Lighthouse — подробности в статье
   [«Lighthouse 100%»](/blog/lighthouse-100). Используйте DevTools, чтобы
   отследить пропущенные кадры
