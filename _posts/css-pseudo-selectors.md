---
title: 'CSS Псевдоклассы и псевдоэлементы: полное руководство'
excerpt:
...
создают виртуальные элементы, которых нет в разметке. Вместе они закрывают то,
ради чего раньше добавляли лишние теги или писали JavaScript.
...
Стилизовать элемент по тому, что лежит внутри него, раньше можно было только из
JavaScript. Теперь это одна строка CSS.
...
[кодировщике SVG для CSS](/tools/svg-to-base64-encoder), чем вручную заменять
...
1. Псевдоэлементы создают реальные узлы и попадают в render tree.
2. Разросшиеся селекторы замедляют рендеринг, держите специфичность в разумных
   пределах.
3. Тени и фильтры в `::before` и `::after` обходятся дорого.
4. CSS containment ограничивает область пересчёта стилей.
...
На псевдоклассах и псевдоэлементах держится изрядная часть повседневного CSS. С
ними делают:
...
- нормальный фокус с `:focus-visible`.

### Что запомнить

1. Для фокуса берут `:focus-visible`, а не `:focus`, и обводку не удаляют.
2. `:has()` наконец-то даёт стилизовать родителя по содержимому.
3. `::before` и `::after` держат на себе всю декорацию, но требуют `content`.
4. Псевдоклассы комбинируются, и этим стоит пользоваться.
5. Длинные цепочки селекторов бьют по
   [скорости рендеринга](/blog/lighthouse-100).
